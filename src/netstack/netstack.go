...
type UQDANetstack struct {
	stack *stack.Stack
}

func CreateUQDANetstack(ygg *core.Core) (*UQDANetstack, error) {
	s := &UQDANetstack{
	...
	if err := s.NewUQDANIC(ygg); err != nil {
		return nil, fmt.Errorf("s.NewUQDANIC: %s", err.String())
	}
	return s, nil
}
...
func (s *UQDANetstack) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
...
func (s *UQDANetstack) DialTCP(addr *net.TCPAddr) (*gonet.TCPConn, error) {
	fa, pn, _ := convertToFullAddr(addr.IP, addr.Port)
	return gonet.DialTCP(s.stack, fa, pn)
}

func (s *UQDANetstack) DialUDP(addr *net.UDPAddr) (*gonet.UDPConn, error) {
	fa, pn, _ := convertToFullAddr(addr.IP, addr.Port)
	return gonet.DialUDP(s.stack, nil, &fa, pn)
}

func (s *UQDANetstack) ListenTCP(addr *net.TCPAddr) (net.Listener, error) {
	fa, pn, _ := convertToFullAddr(addr.IP, addr.Port)
	return gonet.ListenTCP(s.stack, fa, pn)
}

func (s *UQDANetstack) ListenUDP(addr *net.UDPAddr) (*gonet.UDPConn, error) {
	fa, pn, _ := convertToFullAddr(addr.IP, addr.Port)
