...
func isUQDAAddress(ip net.IP) bool {
	ip = ip.To16()
	return ip != nil && ip[0]&0xfe == 0x02 // 0200::/7
}
...
func (m *TCPLocalMappings) Set(value string) error {
	...
	// Second address can be only UQDA ipv6

	if !strings.Contains(second_address, ":") {
		return fmt.Errorf("UQDA listening address can be only IPv6")
	}
	...
	if second_address != "" {
		...
		if !isUQDAAddress(mappedaddr) {
			return fmt.Errorf("mapped address %q is outside the UQDA 0200::/7 range", second_address)
		}
...
func (m *TCPRemoteMappings) Set(value string) error {
	...
	if first_address != "" {
		return fmt.Errorf("UQDA listening must be empty")
	}
